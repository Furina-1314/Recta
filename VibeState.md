...
- **2026-09-25 晚 · 交互细节四项 + 使用指南** ① **列表行命中修复**：自绘 ListBoxItem 模板的选择底色 Border 未悬停时背景为 null——Avalonia 中 null 背景不参与命中测试,悬停行内空白处事件穿透、仅文字可触发;改 `Background="Transparent"`(可命中、不可见),全列表生效。② **弹窗列表裁剪根治**：去掉两处内嵌 MaxHeight ScrollViewer(嵌套滚动冲突致末行永被裁),列表直接铺开由对话框整体滚动。③ **"大盘"更名"总览"** + 六个页面大标题下的解释性小字全部移除。④ **新增 `docs/使用指南.md`**：安装与首次配置、窗口标识(测试库/生产)、角色权限表、七模块操作说明(提单/审批/办结/筛选/导出/批量充值)、实时同步与断线、常见问题(登录失败/无引导/离线/崩溃日志位置);README 目录表挂链。⑤ 踩坑记录：python re.sub 替换串中的 `` 被当反向引用写入 0x01 字节致 XAML 损坏(需按字节修复)。
