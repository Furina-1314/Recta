...
- **2026-09-25 · 测试方法论文档化** 生产零污染的三轨测试体系落成并写入 README"测试与数据库隔离"：①自动化回归（C++ 46 项 + C# 6 项，全部经 .env.test.local/RECTA_TEST_DATABASE_URL 打 recta-test 分支，测试内部自清库自建数）；②GUI 手工试玩——新增 `scripts/launch-test.bat`（读 .env.test.local 设置 DATABASE_URL 环境变量后启动应用，环境变量优先级高于 .env.local，故该实例所有写入均落测试分支）；③正式使用走 production。配套 `db/reset-test.sql`（Neon Console 执行重置测试数据）与 Neon 分支 Reset from parent 说明。开发默认构建 RECTA_DEV_TOOLS=ON（全栈测试依赖），发布打包前切 OFF。
