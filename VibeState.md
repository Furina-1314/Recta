...
- **最新 Release**：v0.3.0（2026-09-25，稳定性+审批人/办结人显示+测试体验；v0.2.0/v0.1.0/v0.0.1 为历史版本）
...
| **P13** | 打包收尾：发布配置、README 截图与构建说明、版本 tag | ✅ 完成 | 自包含发布产物冒烟通过 |
| **Release** | EXE 安装包(Inno Setup)与 GitHub Release 发布流程 | ✅ v0.1.0 已发布 | 安装/运行/卸载闭环验证；v0.1.0 tag 已移至最新代码 |
...
- **2026-09-25 · CSV 导出功能** 应需求新增三处导出（此前无任何导出能力）：①账目页[导出]——当前筛选视图下的全部流水（单号/类型/渠道/事项来源/金额/日期）；②分户页[导出]——名单与余额（学号/姓名/余额）；③分户详情"导出此流水"——选中同学的历次流水（日期/类型/变动/变动后余额/备注）。实现：`CsvExport`（RFC 4180 转义、CRLF、**UTF-8 带 BOM** 保证 Excel 直开中文不乱码），系统保存对话框（文件名自动带时间戳）；导出范围与页面可见性一致（生活委员仅班费类）。测试：新增 xunit `CsvExportTests` 11 项（转义矩阵、BOM 字节、CRLF、逗号/引号字段、**TextFieldParser 按 RFC 4180 往返解析**），全量 17 项真跑全绿。
- **2026-09-25 · 驳回结构化 + 数据库重置** ① **驳回改为预置原因下拉(含"其他")+补充说明**：新增域层 `RejectReasons.hpp`(7 项键值/中文标签单一事实来源：票据凭证不全、金额有误、不属于班级支出、超出预算额度、信息填写不完整、重复提单、其他)，`RejectRequest(actor,id,category,notes)` 校验——原因必选、非法键拒绝、**"其他"必须附补充说明**；入库列 `expense_requests.reject_category`(键)，`review_notes` 语义变为 审批意见/核减原因/驳回补充说明。**常见驳回成因统计改为按预置键精确分组**(GROUP BY reject_category,CAPI 映射中文标签)——彻底解决原文相似文本被拆分计数的问题，上限放宽至 7(全部预设)。GUI：审批页 Inspector 增"驳回原因"下拉；详情驳回结果显示"[标签]+补充说明"。② **数据库重建**（应用户要求不保留原生产数据）：两分支 DROP 全部 8 表后按新 001 重建(001 并入 voucher_url+reject_category，删除 002 迁移)；production 现为空库，**应用下次启动将重新出现首任团支书引导**(原团支书/生活委员账号已随数据清除，需重新开立)。③ 顺带修复：C# 全栈测试此前因 DLL 为 OFF 构建静默跳过的隐患——测试前须确认 RECTA_DEV_TOOLS=ON(CMakeCache 已验证)。全量回归：C++ 3 套 + C# 6 项(1m05s 真跑)全绿。
- **2026-09-25 晚 · 测试库使用答疑** 用户在 bat 实例用生产账号 Fzl 登录失败且未见引导——均属正确行为：自动化测试每次运行会清空并重建测试账号（当前为 sync_sec，密码 SyncPass1234!），用户表非空则无引导面板，且生产账号只存在于生产库。已重置测试库为空，bat 实例下次启动将出现首任引导；**注意：再跑一次 `dotnet test`/ctest 后测试库会重新被测试账号占据，需再执行 db/reset-test.sql 恢复引导状态**。
- **2026-09-25 晚 · 交互细节四项 + 使用指南** ① **列表行命中修复**：自绘 ListBoxItem 模板的选择底色 Border 未悬停时背景为 null——Avalonia 中 null 背景不参与命中测试,悬停行内空白处事件穿透、仅文字可触发;改 `Background="Transparent"`(可命中、不可见),全列表生效。② **弹窗列表裁剪根治**：真因是外层对话框滚动区与内层列表滚动区**嵌套**(滚轮事件与测量高度互相干扰,内层滚不到底、末行恒被裁);修复为**移除外层对话框滚动、只保留列表自身内嵌滚动**(单一滚动上下文,MaxHeight=240+底部留白,滚到底必然完整显示末行)——应 developers 询问确认:内嵌滚动可以保留,关键是消除嵌套。③ **"大盘"更名"总览"** + 六个页面大标题下的解释性小字全部移除。④ **新增 `docs/使用指南.md`**：安装与首次配置、窗口标识(测试库/生产)、角色权限表、七模块操作说明(提单/审批/办结/筛选/导出/批量充值)、实时同步与断线、常见问题(登录失败/无引导/离线/崩溃日志位置);README 目录表挂链。   事后补遗：脚本批处理曾误删 5 个页面 XAML 中的合法多行元素(审批意见/办结批复标签、TempPasswordLabel、内嵌 StackPanel、外层 </Grid> 等),导致 XAML 损坏与 ApproveAmountBox 丢失——已逐一人工修复并用 XML 解析器全量体检(10 个 axaml 全部良构);教训:**对结构化文件禁用"匹配特征行删除"式脚本批改,必须改后跑 XML/编译双重校验**。⑤ 踩坑记录：python re.sub 替换串中的 `` 被当反向引用写入 0x01 字节致 XAML 损坏(需按字节修复)。
- **2026-09-25 晚 · 提单弹窗修显与批量充值** ① **显示修复**：尾差承担人/充值对象/关联报销单三个 ComboBox 因 Avalonia 无 DisplayMemberPath 而落到默认 ToString(显示 StudentCheckVm/StudentComboVm 类名)——改 ItemTemplate 绑定 Label；参摊/充值同学列表滚动区加高(170)+底部留白，末行不再被裁。② **全选**：参摊名单与充值对象各配"全选"复选框，三态(全选/全不/部分半选)，与单项勾选双向联动。③ **班费批量充值**：域层新增 `RecordInflowBatch`(仅生活委员、单事务、学号去重与存在性预校验、逐笔 ApplyCredit+inflow+ledger+事件、守恒强校验失败整体回滚、返回人数)；CAPI `recta_record_inflow_batch` + C# `RecordInflowBatch`；入账登记"同学班费充值"改为多选列表(独立勾选状态,与支出平摊互不串扰)。**新增 C++ 测试**：批量原子性(两人成功+含幽灵学号整体回滚且余额不变+重复学号/空名单/越权拒绝)全绿。
- **2026-09-25 晚 · 稳定性三轮(用户实测反馈)** ① **卡退根因(事件日志实锤)**：大盘页刷新崩溃——`OverviewPage.LoadAsync` 中 `(IBrush?)Application.Current!.FindResource(...)` 返回 UnsetValueType 强转抛 InvalidCastException,经 async void 未处理直接终止进程(WER/.NET Runtime 事件栈确认);状态分布"一直为零"即同源(崩溃点在计数赋值之前)。**修复**：徽章改双 Border 纯 XAML DynamicResource 切换(IsVisible),零资源查找;全局清点后 code-behind 已无任何直接强转资源查找。**兜底**：Dispatcher.UIThread.UnhandledException 记录崩溃日志(%LOCALAPPDATA%\Recta\logs\crash.log)并保持运行。② **卡顿治理**：NeonContext 连接复用——按实例缓存一条连接(mutex 串行化,桌面单用户足够),免除每次事务的 TCP+TLS 握手(跨城数百毫秒);断线重建+退避,事务异常丢弃连接保证干净;同步 worker 拉取同样复用。C++ 回归 290s(套件本身因复用提速近半),C# 全栈+同步收敛真跑 38s。③ **审批人/办结人显示**：审批页 Inspector 增提单人/审批人/办结人三栏;账目详情增审批人/办结人独立字段(不再仅随批复文本出现)+驳回原因结构化展示。
- **2026-09-25 晚 · 测试实例连生产缺陷根治（用户实测报告"bat 启动后 Fzl 仍能登录"）** 三层排查定位真因：①bat 的 `for /f` 解析 `.env.test.local`（含 UTF-8 中文注释）在 GBK 代码页下乱码字节破坏解析块——**实测 `DATABASE_URL` 从未被设置**（dump 复现），应用回退到 cwd 上溯的 `.env.local`（生产）；②叠加用户此前双击 exe 直接登录（last_login 12:26Z 写在生产），形成"测试实例能登生产号"的假象。**修复**：(a) bat 放弃解析文件，改设两个纯 ASCII 无条件变量——`RECTA_TEST_MODE=1`（UI 标记）+ `RECTA_ENV_FILE=<绝对路径指向 .env.test.local>`，由原生侧（UTF-8 无忧）读取；(b) EnvConfig 优先级重构为显式四级：DATABASE_URL env → RECTA_ENV_FILE → 测试模式到此为止（**硬守卫：测试实例禁止回退 .env.local，测试文件缺失宁可失败**）→ 常规 .env.local 上溯；(c) 新增 2 项 StorageTests 锁定行为（RECTA_ENV_FILE 覆盖生效、测试模式拒绝 .env.local 回退）——期间踩坑：Win32 `SetEnvironmentVariableA` 的运行期改动 `getenv`（CRT 副本）看不到，测试必须用 `_putenv`；(d) F:\Recta 已就地更新为修复版载荷，ctypes 复刻实验证实应用现读测试分支。测试分支数据已重置为空，bat 实例下次启动将出现首任引导。
- **2026-09-25 · 自保护审计与 UI 补强** 应用户询问审计账号自保护措施——**域层防护齐备且有测试覆盖**：①停用自己被拒("团支书不可停用自身账号")；②重置自己密码被拒(必须走验旧口令的修改密码,防自锁)；③团支书/生活委员席位唯一(开立与启用双向校验,不可出现第二在任者)；④系统引导仅空库可用一次；⑤停用账号立即无法登录；⑥账号只有停用没有删除(无数据丢失面)。**UI 补强**：用户页"本人"行隐藏 重置/改名/停用 三按钮并标注"（本人）"(此前点了会被服务端拒绝报错,现在直接不提供)；域层拒绝仍在(双保险)。**自审自批为确认允许的设计**(2026-09-25 用户拍板):团支书提班费单可自审,生活委员自审系报销单为唯一审批人路径——不引入审批分离。
- **2026-09-25 · 测试方法论文档化** 生产零污染的三轨测试体系落成并写入 README"测试与数据库隔离"：①自动化回归（C++ 46 项 + C# 6 项，全部经 .env.test.local/RECTA_TEST_DATABASE_URL 打 recta-test 分支，测试内部自清库自建数）；②GUI 手工试玩——`scripts/launch-test.bat`（读 .env.test.local 设置 DATABASE_URL 环境变量后启动应用，环境变量优先级高于 .env.local，故该实例所有写入均落测试分支；**2026-09-25 重写为纯 ASCII**——cmd 按 GBK 解析含中文的 bat 会导致整脚本乱码失效（第一版即因此从未真正启动过测试实例，用户误以生产窗口为测试窗），与 PS1 同款编码坑；应用查找顺序：参数 1 → F:\Recta（用户安装位）→ 仓库开发构建 → %LOCALAPPDATA% 安装位；**bat 额外设 RECTA_TEST_MODE=1，应用标题栏显示【测试库】**（生产实例无标记），实测双向区分有效；F:\Recta 已就地更新至含标记的构建）；③正式使用走 production。配套 `db/reset-test.sql`（Neon Console 执行重置测试数据）与 Neon 分支 Reset from parent 说明。开发默认构建 RECTA_DEV_TOOLS=ON（全栈测试依赖），发布打包前切 OFF。
